...
import { E as SqliteDialect, L as sql, U as Kysely, d as MssqlDialect, g as PostgresDialect, x as MysqlDialect } from "./dist-Fc1YC-0v.js";
import { n as createAdapterFactory } from "./adapter-BAZSqCpv.js";
...
var createKyselyAdapter = async (config) => {
	...
	if ("fileControl" in db) {
		const { BunSqliteDialect } = await import("./bun-sqlite-dialect-C7ftEUSI-C1qKJIbV.js");
	...
	if ("createSession" in db) {
		...
		if (DatabaseSync && db instanceof DatabaseSync) {
			const { NodeSqliteDialect } = await import("./node-sqlite-dialect-D0lNXakw.js");
			dialect = new NodeSqliteDialect({ database: db });
			transaction = true;
		}
	}
	if ("batch" in db && "exec" in db && "prepare" in db) {
		const { createD1IndexIntrospector, D1SqliteDialect } = await import("./d1-sqlite-dialect-D4qp4-wW-CD-MNeBG.js");
