...
function snapshotSharedJWE(jwe) {
	...
	if (iv !== void 0 && typeof iv !== "string") throw new JWEInvalid("JWE Initialization Vector incorrect type");
	if (typeof ciphertext !== "string") throw new JWEInvalid("JWE Ciphertext missing or incorrect type");
	if (tag !== void 0 && typeof tag !== "string") throw new JWEInvalid("JWE Authentication Tag incorrect type");
...
function snapshotRecipientJWE(recipient) {
	...
	try {
		...
		if (isObject(inputHeader)) {
			...
			if (!parameters.includes("alg")) headerAlg = void 0;
			header = Object.fromEntries(parameters.map((parameter) => [parameter, parameter === "alg" ? headerAlg : inputHeader[parameter]]));
...
function checkRecipient(jwe) {
	...
	if (header !== void 0) {
		if (!isObject(header)) throw new JWEInvalid("JWE Per-Recipient Unprotected Header incorrect type");
	}
...
function shareJWE(jwe) {
	...
	let parsedProt;
	if (encodedProtected !== void 0) parsedProt = parseJoseHeader(encodedProtected, JWEInvalid, "JWE Protected Header is invalid");
...
async function decryptCompact(jwe, shared, key) {
	...
	const protectedBytes = encode$1(protectedHeader);
...
var GeneralEncrypt = class {
	...
	async encrypt() {
		...
		for (let i = 0; i < this.#recipients.length; i++) {
			const [, keyManagementParameters, key] = this.#recipients[i].state;
			const target = {};
...
function snapshotJws(jws, sharedPayload) {
	...
	if (!sharedPayload && payload instanceof Uint8Array) payload = new Uint8Array(payload);
	const signature = jws.signature;
	const snapshot = {
		payload,
		signature
	};
	if (encodedProtected !== void 0) snapshot.protected = encodedProtected;
	if (inputHeader !== void 0) snapshot.header = header;
	return snapshot;
}
...
async function verifySignature(jws, shared, key, encodeUnencodedPayload, parsedProtected) {
	...
	return verifyPrepared(jws, shared, key, encodedProtected, parsedProt, alg, b64 || typeof inputPayload !== "string" ? inputPayload : encodeUnencodedPayload(inputPayload));
}
...
async function flattenedVerify(jws, key, options) {
	...
	if (snapshot.protected === void 0 && snapshot.header === void 0) throw new JWSInvalid("Flattened JWS must have either of the \"protected\" or \"header\" members");
	if (snapshot.protected !== void 0 && typeof snapshot.protected !== "string") throw new JWSInvalid("JWS Protected Header incorrect type");
	if (snapshot.payload === void 0) throw new JWSInvalid("JWS Payload missing");
	if (typeof snapshot.signature !== "string") throw new JWSInvalid("JWS Signature missing or incorrect type");
	if (snapshot.header !== void 0 && !isObject(snapshot.header)) throw new JWSInvalid("JWS Unprotected Header incorrect type");
...
function snapshotSignature(signature, payload) {
	try {
		...
		if (encodedProtected === void 0 && header === void 0) return void 0;
		if (encodedProtected !== void 0 && typeof encodedProtected !== "string") return void 0;
		if (typeof encodedSignature !== "string") return void 0;
		if (header !== void 0 && !isObject(header)) return void 0;
...
async function generalVerify(jws, key, options) {
	...
	for (const [, , mode] of candidates) {
		modes |= mode;
		if (modes === 3) throw new JWSInvalid("inconsistent use of JWS Unencoded Payload (RFC7797)");
	}
...
async function createSignature(input, key, assertB64) {
	...
	if (unprotectedHeader !== void 0) unprotectedHeader = serializeJoseHeader(JWSInvalid, unprotectedHeader)[0];
...
var UnsecuredJWT = class extends UnsecuredJWT_base {
	...
	static decode(jwt, options) {
		...
		try {
			...
			if (!(cause instanceof JWSInvalid)) throw cause;
...
async function generateKeyPair(alg, options) {
	...
	return crypto.subtle.generateKey(algorithm, extractable ?? false, [...entry.usages[1], ...entry.usages[0]]);
}
//#endregion
...
//#endregion
export { generalVerify as A, GeneralSign as C, CompactEncrypt as D, FlattenedEncrypt as E, flattenedDecrypt as F, compactDecrypt as I, base64url_exports as L, compactVerify as M, GeneralEncrypt as N, jwtDecrypt as O, generalDecrypt as P, encode as R, SignJWT as S, CompactSign as T, calculateJwkThumbprintUri as _, decodeProtectedHeader as a, exportSPKI as b, importSPKI as c, createRemoteJWKSet as d, customFetch as f, calculateJwkThumbprint as g, EmbeddedJWK as h, decodeJwt as i, flattenedVerify as j, jwtVerify as k, importX509 as l, createLocalJWKSet as m, generateSecret as n, importJWK as o, jwksCache as p, generateKeyPair as r, importPKCS8 as s, cryptoRuntime as t, UnsecuredJWT as u, exportJWK as v, FlattenedSign as w, EncryptJWT as x, exportPKCS8 as y };
