...
export function XpCard({ className }: { className?: string }) {
  ...
  return (
    <Link
      to="/account"
      className={cn(
        "flex flex-col justify-center rounded-card bg-surface p-card shadow-[var(--elevation-1)] transition-[box-shadow] duration-[var(--duration-fast)] ease-[var(--ease-out)] hover:shadow-[var(--elevation-2)] active:shadow-[var(--elevation-2)]",
        className,
      )}
    >
      <p className="flex items-center gap-1.5 text-xs font-medium tracking-wide text-muted uppercase">
