...
/**
 * Same as `authMiddleware`, but for server functions a signed-out visitor is
 * allowed to call too (e.g. reading a set that might be public) —
 * `context.userId` is the verified user id, or `null` when signed out.
 * The handler is responsible for its own authorization (e.g. only returning
 * data for `isPublic` rows when `userId` doesn't match the owner).
 */
